...
// NewPrometheusObserver builds the observer on a private registry and returns it plus the

// HTTP handler that scrapes that registry (mounted at /metrics by the serve command). The

// version string populates gantry_build_info; pass the CLI version (set by ldflags).
...
func NewPrometheusObserver(version string) (*PrometheusObserver, http.Handler) {
	...
	reg.MustRegister(
		...
		o.reconcile, o.duration, o.deploys, o.rollbacks, o.verifyFails, o.driftAge, buildInfo,
	)

	return o, promhttp.HandlerFor(reg, promhttp.HandlerOpts{})
}

// ReconcileDone records one reconcile's outcome. The duration is always observed; the

// result label is one of deployed/failed/nochange; deploys, verify failures, and auto

// rollbacks increment their own counters when they occur.
...
// DriftObserved records the age of a drifted component (seconds) — last-observed wins.
